...
async def menu_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "events":
        ...
        for ename,m in all_m[-10:]:
            ...
            text += f"*{m['home']}* {sh}:{sa} *{m['away']}*{date_str}\n{res} · _{ename}_\n\n"
        await q.edit_message_text(text, parse_mode="Markdown", reply_markup=back_kb())
...
async def ev_confirm_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(
        f"{status}\n\n🎉 *{ev['name']}* создано!\n\nВводи имена участников, /done — завершить",
        parse_mode="Markdown"
    )
    return ADD_PARTICIPANT
...
async def ap_name(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    status = "✅" if ok else "⚠️"
    await update.message.reply_text(f"{status} *{name}* добавлен\nСписок: {current}\n\nЕщё или /done", parse_mode="Markdown")
    return ADD_PARTICIPANT
...
async def match_score_msg(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    event["matches"].append({
        ...
        "played":True,
        "date":m.get("date", datetime.now().strftime("%d.%m.%Y"))
    })
    ok = save_data(data)
    res = f"🏆 *{m['home']}*" if sh>sa else (f"🏆 *{m['away']}*" if sa>sh else "🤝 Ничья")
    status = "✅ Результат сохранён!" if ok else "⚠️ Ошибка сохранения"
    adm = is_admin(update.effective_user.id, data)
    await update.message.reply_text(
        f"{status}\n\n*{m['home']}* {sh}:{sa} *{m['away']}*\n{res}\n📅 {m.get('date','')}",
...
async def edit_event_name_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(f"Текущее название: *{event['name']}*\n\nВведи новое название:", parse_mode="Markdown")
...
async def edit_event_name_value(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    event["name"] = new_name
    ok = save_data(data)
    status = "✅ Название изменено!" if ok else "⚠️ Ошибка"
    adm = is_admin(update.effective_user.id, data)
    await update.message.reply_text(
        f"{status}\n\n*{old_name}* → *{new_name}*",
        parse_mode="Markdown", reply_markup=main_menu_kb(adm)
...
async def edit_match_event_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [[InlineKeyboardButton(
        ...
        callback_data=f"emm_{i}"
    )] for i,m in enumerate(event["matches"]) if m.get("played")]
    kb.append([InlineKeyboardButton("◀️ Назад", callback_data="back_main")])
    await q.edit_message_text("⚽ Выбери матч для редактирования:", reply_markup=InlineKeyboardMarkup(kb))
...
async def edit_match_select_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    m = event["matches"][idx]
    await q.edit_message_text(
        f"Текущий счёт: *{m['home']}* {m['score_home']}:{m['score_away']} *{m['away']}*\n\nВведи новый счёт (формат: `2:1`):",
        parse_mode="Markdown"
    )
...
async def edit_match_score_value(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    data = load_data()
    event = get_event(data, ctx.user_data["edit_eid"])
    idx = ctx.user_data["edit_match_idx"]
    m = event["matches"][idx]
    old = f"{m['score_home']}:{m['score_away']}"
    m["score_home"] = sh; m["score_away"] = sa
    ok = save_data(data)
    status = "✅ Счёт изменён!" if ok else "⚠️ Ошибка"
    adm = is_admin(update.effective_user.id, data)
    await update.message.reply_text(
...
async def edit_participant_event_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    kb.append([InlineKeyboardButton("◀️ Назад", callback_data="back_main")])
    await q.edit_message_text("👤 Выбери участника для переименования:", reply_markup=InlineKeyboardMarkup(kb))
...
async def edit_participant_new_value(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if old_name in event["participants"]:
        idx = event["participants"].index(old_name)
        event["participants"][idx] = new_name
    # Update in matches too
    for m in event.get("matches",[]):
        if m.get("home") == old_name: m["home"] = new_name
        if m.get("away") == old_name: m["away"] = new_name
    ok = save_data(data)
    status = "✅ Имя изменено!" if ok else "⚠️ Ошибка"
    adm = is_admin(update.effective_user.id, data)
    await update.message.reply_text(
        f"{status}\n\n*{old_name}* → *{new_name}*\n(обновлено и в матчах)",
        parse_mode="Markdown", reply_markup=main_menu_kb(adm)
    )
    return ConversationHandler.END
...
async def remove_participant_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    ok = save_data(data)
    status = "✅ Участник удалён!" if ok else "⚠️ Ошибка"
    adm = is_admin(q.from_user.id, data)
    await q.edit_message_text(f"{status}\n\n*{name}* удалён из события.", parse_mode="Markdown", reply_markup=main_menu_kb(adm))
    return ConversationHandler.END
...
async def finish_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if event:
        event["active"] = False
        ok = save_data(data)
        status = "✅ Событие завершено!" if ok else "⚠️ Ошибка"
...
async def delete_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if event:
        ...
        ok = save_data(data)
        status = "✅ Событие удалено!" if ok else "⚠️ Ошибка"
        adm = is_admin(q.from_user.id, data)
...
async def add_admin_msg(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if new_id not in data["admins"]:
        ...
        status = "✅ Админ добавлен!" if ok else "⚠️ Ошибка"
...
async def reset_cb(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "reset_yes":
        ...
        status = "✅ Все данные очищены!" if ok else "⚠️ Ошибка"
        await q.edit_message_text(status, reply_markup=main_menu_kb(True))
    else:
        await show_main_menu(q, ctx)
    return ConversationHandler.END
...
def main():
    ...
    print("✅ ShymATC-CHAMP bot v3 started")
